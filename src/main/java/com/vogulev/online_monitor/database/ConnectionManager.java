...
public class ConnectionManager
{
     ...
     */
    public void connect(FileConfiguration config, File dataFolder)
    {
        this.databaseType = config.getString("database.type", "sqlite").toLowerCase();

        Properties props = loadHikariProperties(config);
        ...
        {
            String driver = MYSQL.equals(databaseType)
     ...
     */
    private Properties loadHikariProperties(FileConfiguration config)
    {
        Properties props = new Properties();

        ConfigurationSection hikariSection = config.getConfigurationSection("database.hikari");

        if (hikariSection != null)
        {
            for (String key : hikariSection.getKeys(false))
            {
                Object value = hikariSection.get(key);
                if (value != null)
                {
     ...
     */
    private String buildJdbcUrl(FileConfiguration config, File dataFolder)
    {
        if (MYSQL.equals(databaseType))
        {
            String host = config.getString("database.mysql.host", "localhost");
            int port = config.getInt("database.mysql.port", 3306);
            String database = config.getString("database.mysql.database", "minecraft_stats");
            String timezone = config.getString("database.mysql.timezone", "UTC");
        ...
        {
            String filename = config.getString("database.sqlite.filename", "statistics.db");
    ...
    private void runMigrations()
    {
        try
        {
            ...
            if (result.migrationsExecuted > 0)
            {
                ...
                String version = result.targetSchemaVersion != null ? result.targetSchemaVersion : "baseline";
